...
We can see in this area there were `r manchester %>% filter(code=="E01033658") %>% pull(count)` crime incidents for a residential population of `r manchester %>% filter(code=="E01033658") %>% pull(respop)`, but the workday population is as high as `r manchester %>% filter(code=="E01033658") %>% pull(wkdpop)` people. So, of course, using these different denominators is bound to have an impact in the resulting rate. As noted earlier, using as a denominator some measure of the population at risk is most appropriate.
