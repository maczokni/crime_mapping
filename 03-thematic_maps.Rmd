...
In this chapter we will get to know how to think about thematic maps, and how to apply your learning to creating your own maps of this variety. In the process we will discuss various types of thematic maps and the issues they raise.
...
In this chapter, we introduce in particular two types of thematic maps used for mapping quantitative variables: **choropleth (thematic) maps** and **proportional symbol maps**. In previous chapters we introduced two separate packages for creating maps in R (`ggplot2` and `leaflet`). In this chapter we will introduce a third package for creating maps: `tmap`. The libraries we use in this chapter are as follow:
...
```{r, message=FALSE, fig.cap='Bubble map of crime in each LSOA in Manchester', fig.height=4}
...
```{r, fig.cap='Bubble map with aesthetic settings', fig.height=4}
...
```{r, fig.cap='Bubble map with perceptual parameter set to TRUE', fig.height=4, warning=FALSE, message=FALSE}
tm_shape(manchester) +  
...
```{r, fig.cap='Histogram of respop variable', fig.height=3}
...
```{r, fig.cap='Histogram of wkdpop variable', fig.height=3}
...
```{r, message=FALSE, fig.cap='Crime rate per residential population', fig.height=4}
tm_shape(manchester) + 
  tm_polygons("crimr1")
```
...
```{r, message=FALSE, fig.cap='Crime rate per residential population mapped with tm_fill()', fig.height=4}
tm_shape(manchester) + 
  tm_fill("crimr1")
```
...
```{r, message=FALSE, fig.cap='Crime rate per residential population mapped with tm_fill() and borders', fig.height=4}
tm_shape(manchester) + 
  tm_fill("crimr1") +
  tm_borders(alpha = 0.1)
```
...
```{r, message=FALSE, fig.cap='Crime rate per residential population mapped with aesthetics considered', fig.height=4}
...
We can also change the style of the maps we produce, for example by making them more friendly to colour blind people. We can use the `tmap_style()` function to do so.
...
```{r, fig.cap='Crime rate per residential population with col_blind style', fig.height=4}
...
```{r, echo=FALSE, fig.cap='Results for equal intervals classification', fig.height=4}
...
```{r, echo=FALSE, fig.cap='Results for quantiles classification', fig.height=4}
pal1 <- c("wheat1", "red3")
...
```{r, echo=FALSE, fig.cap='Results for Jenks classification', fig.height=4}
pal1 <- c("wheat1", "red3")
...
```{r, echo=FALSE, fig.cap='Results for standard deviation classification', fig.height=4}
pal1 <- c("wheat1", "red3")
...
```{r, echo = FALSE, fig.cap='Results for headtails classification', fig.height=4}
pal1 <- c("wheat1", "red3")
...
```{r, fig.cap='Quantile classification of crime in Manchester', fig.height=4}
...
```{r, message=FALSE, fig.cap='Compare maps using the different classifications of crime in Manchester'}
...
```{r, cache=FALSE, fig.cap='Interactive Jenks map of crime by residential population in Manchester', fig.height=3}
...
```{r, cache=FALSE, fig.cap='Interactive Jenks map of crime rate by workday population in Manchester', fig.height=3}
tm_shape(manchester) + 
...
To answer this, let's look at the population of Borden county in the 1960s, contained in the `PO60` variable. 
...
Instead of shrinking to the global rate, we can shrink to a rate based on the neighbours of each county. Shrinking to the global rate ignores the spatial dimension of the phenomenon being mapped out and may mask existing heterogeneity. If instead of shrinking to a global rate, we shrink to a local rate, we may be able to take unobserved heterogeneity into account. @Marshall_1991 proposed a local smoother estimator in which the crude rate is shrunk towards a local, "neighbourhood", rate. To compute this we need the list of neighbours that surround each county (we will discuss this code in the Chapter on Spatial Dependence, so for now just trust us we are computing the rate of the areas that surround each country):
...
```{r, message=FALSE, warning=FALSE, fig.cap='Compare raw and smoothed rates across counties in the USA'}
tmap_mode("plot")
current_style <- tmap_style("col_blind")
...
```{r, fig.cap='Boxplots to compare variation in rates', fig.width=4}
