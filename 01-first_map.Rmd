...
A word of caution about WGS 84. Although it is very popular, it has a problem. It is fixed to the North America plate. And plates do move about 2.5 cm per year. If you work in North America you should not worry then, but if you work elsewhere this is an issue. There has been movement since the datum was created and thus we can correct for this, but this means that we need to know the time of our spatial feature (say the time of a criminal event) if we want to the correction needed as a result of plate tectonics. The problem is not simply of data availability (say you don´t have the exact timing of the event that you want to place in a map), but of data structures. Traditionally the file formats that we have used to store this information do not lead themselves easily to record this information so that we can then easily address these inaccuracies. Progress has been made in recent times, but you need to be aware that WGS84 is not perfect despite its popularity. Some European guidelines suggest the use of ETRS89 in Europe as a consequence of this.
