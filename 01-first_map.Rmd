...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.height=3, fig.width=6, fig.cap = 'Point, Line, and Polygon'}
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.height=3, fig.width=6, fig.cap = 'Layers of ggplot'}
...
```{r, echo = FALSE, message=FALSE, warning=FALSE, fig.cap='Final plot', fig.height=3}
...
```{r, fig.cap='Data layer', fig.height=3}
library(ggplot2)
...
```{r, fig.cap='Geometry layer', fig.height=3}
...
```{r, fig.cap='Annotations layer', fig.height=3}
...
```{r, fig.cap='Customise themes', fig.height=3}
...
```{r, fig.cap='Additional layers', fig.height=3}
...
```{r, fig.cap='Customise colour', fig.height=3}
...
```{r, fig.cap='geom_sf for plotting', fig.height=3}
...
```{r, message=FALSE, fig.cap='Adding a basemap', fig.height=3}
...
```{r, echo = FALSE, message=FALSE, warning=FALSE, results='hide', fig.cap='Hierarchy of Census Geographies in the UK', fig.height=3}
...
Now use the `st_read()` function from the `sf` package to read in the shapefile (the one with the `.shp` extension: 
...
```{r, fig.cap="Plot of Manchester's LSOAs", fig.height=3}
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.cap='Two dataframes with common unique identifier'}
library(grid)
library(gridExtra)
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.cap='Joined dataframe'}
...
This is all we are doing, when merging tables: joining the two data sets using a common column.
...
Remember our original empty map? Well now, since we have the column (variable) for number of crimes here, we can use that to shade the polygons based on how many crimes there are in each LSOA. We can do this by specifying the `fill=` parameter of the `geom_sf` function. 

```{r, fig.cap='Number of crimes in Manchester by LSOA'}
...
```{r, message=FALSE, fig.cap='Number of crimes by LSOA with adjusted parameters'}
