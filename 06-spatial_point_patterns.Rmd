...
# Basic reading and manipulation of spatial data
library(readr)
library(sf)
library(dplyr)
library(maptools)
...
It is important to realise that a point pattern is defined as a series of events in a given area, or window, of observation. Therefore we need to precisely define this window. In `spatstat` the function `owin()` is used to set the observation window. However, the standard function takes the coordinates of a rectangle or of a polygon from a matrix, and therefore it may be a bit tricky to use. Luckily the package `maptools` provides a way to transform a `SpatialPolygons` into an object of class `owin`, using the function `spatstat.geom::as.owin()`. Here are the steps. 
...
```
As noted above, EPGS:32118 uses meters. So we can include this information to our `owin` object:
...
Now we can assign to this `ppp` object the numerical vector as the *mark* using the `marks` function.
...
Meters are a small unit. So the value we get is very close to zero. We can use the `rescale` function to obtain the intensity per km instead. The second argument is the number we multiply our original units by:
...
If we try this test with the tesselation we created, we will get a warning about cells with small counts. This test...
...
The key argument to pass to the density method for point patterm objects is `sigma=`, which determines the bandwidth of the kernel. Sigma is the standard deviation of the kernel and you can enter a numeric value or a function that computes an appropriate value for it.

In  the`spatstat.core` package the functions `bw.diggle()`, `bw.ppl()`, and `bw.scott()` can be used to estimate the bandwidth according to difference methods (mean square error cross validation, likelihood cross validation, and Scott´s rule of thumb for bandwidth section in multidimensional smoothing respectively). The help files recommend the use of the first two. These functions run algorithms that aim to select an appropriate bandwidth.
...
Which bandwidth to use? When it comes to crime, @Neville_2013 suggests that the "best choice would be to produce an interpolation that 
fits the experience of the department and officers who travel an area. Again, experimentation and discussions with beat officers will be necessary to establish which bandwidth choice should be used in future interpolations." (p. 10.22) This is generally accepted by other authors in the field.
...
```{r kernels_6}
par(mfrow=c(2,2))
plot(density.ppp(jitter_bur, sigma = 148 ,edge=T),
     main=paste("Gaussian"))
plot(density.ppp(jitter_bur, kernel = "epanechnikov", sigma = 148, edge=T),
     main=paste("Epanechnikov"))
plot(density.ppp(jitter_bur, kernel = "quartic", sigma = 148, edge=T),
     main=paste("Quartic"))
plot(density.ppp(jitter_bur, kernel = "disc", sigma = 148, edge=T),
     main=paste("Disc"))
```
...
```{r}
set.seed(200)
plot(density.ppp(bur_ppp_m, sigma = bw.ppl(bur_ppp_m), edge = T, kernel = "quartic",
...
The plots produced by `spatstat` are good for initial exploration, but if you want to present this to an audience you may to use one of the various packages we have covered so far for visualising spatial data. Here we show how to use `leaflet` for ploting densities.
...
Let´s illustrate both problems. We are going to read a shapefile of NYC census tracts with number of housing units (potential targets for burglary).
...
We are now going to extract the centroids of these polygons so that we can generate a density estimate:

```{r}
nyc_houses <- dplyr::select(housing, value, geometry)
sf_cent <- st_centroid(nyc_houses)

```

Now we are going to create a `ppp` object with these centroids and using the volume of households as a mark.

```{r}

mark <- sf_cent$value
cent_coords <- matrix(unlist(sf_cent$geometry), ncol = 2, byrow = T)
houses_ppp <- ppp(x = cent_coords[,1], y = cent_coords[,2],
                   window = window, check = FALSE, marks = mark)
marks(houses_ppp)
```
...
```{r}
set.seed(200)
b_bur <- BOOT.density(jitter_bur)
b_bur
b_hous <- BOOT.density(houses_ppp)
b_hous
```

```{r}
bivden_bur <- bivariate.density(jitter_bur, h0 = 285.35, adapt = TRUE)
plot(bivden_bur)

```


And let´s check the housing:

```{r}
bivden_hous <- bivariate.density(houses_ppp, h0 = 275, adapt = TRUE,
                                 weights = marks(houses_ppp))
plot(bivden_hous)
```

```{r}
risk(bivden_bur, bivden_hous, doplot=TRUE)
```



We can now plot the density of housing units using the number of housing units as weights:

```{r}
plot(density.ppp(houses_ppp, sigma = 175, 
                 edge = T, kernel = "quartic",
                 weights = marks(houses_ppp)))
```

```{r}
rel_risk <- risk(jitter_bur, houses_ppp, doplot = TRUE)


```
```{r}
hfix <- LSCV.risk(jitter_bur, houses_ppp)
hfix
...
We can also compute a KDE for this:
...
For a deeper understanding of the issues discussed here we recommend the following reading. The documentation manual for CrimeStat (Chapter 10) has a detailed technical discussion of density estimation that is suited for crime analysts (@Levine_2013).
