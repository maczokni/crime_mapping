...
```{r}
ggplot() +
  geom_sf(data = burglary_kde)
```


This should look more like our previous `density.ppp` KDE using similar specs but it doesn´t. Why? The borders of the grid lines are not helping. To avoid this problem we need to tell R that we want the  
