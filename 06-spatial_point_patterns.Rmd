...
## Exploring our point pattern with spatstats
...
For example, if we want four quadrants along the X axis and 3 along the Y axis we could used those parameters in the `spatstat.geom::quadratcount()` function. Then we just use standard plotting functions from R base.
...
```
As you can see with its irregular shape it is hard to divide Manhattan in equally shaped and sized quadrants. We can use other forms of tessellation, like hexagons by using the `hextess` function. Rather than printing the numbers inside these hexagons we can map the intensity to them:
...
```
We can see that areas of Manhattan, like Central Park, have understandably a very low intensity of burglaries, whereas others, like the Lower East Side, have a higher intensity. So this map suggest to us that the process may not be homogeneous - harly surprising when it comes to crime.
...
```{r}
par(mfrow=c(2,2))
plot(density.ppp(jitter_bur, sigma = 537 ,edge=T),
     main=paste("Gaussian"))
plot(density.ppp(jitter_bur, kernel = "epanechnikov", sigma = 537, edge=T),
     main=paste("Epanechnikov"))
plot(density.ppp(jitter_bur, kernel = "quartic", sigma = 537, edge=T),
     main=paste("Quartic"))
plot(density.ppp(jitter_bur, kernel = "disc", sigma = 537, edge=T),
     main=paste("Disc"))
```
...
```{r, message=FALSE}
dmap1 <- density.ppp(jitter_bur, sigma = 537, kernel = "quartic", edge=T)
