...
This is currently just a text dataframe, so we need to let R know that actually this is a spatial object, who's geometry can be find in its longitude and latitude coordinates. As we have long/lat we can know it's in WGS 84 projection. 
...
```{r, fig.height= 3. fig.cap= 'Point map of ASB in Manchester'}
...
```{r, fig.height= 3, fig.cap='Hexbin map of ASB in Manchester'}
...
```{r, fig.height= 3, message=FALSE, fig.cap='Hexbin map of ASB in Manchester with basemap'}
...
- ou will find more neighbours with hexagons than square grids, if you're using a distance band, since the distance between centroids is the same in all six directions with hexagons (see image below). 

```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.height= 1, fig.cap='Hexagonal bins will have more neighbors included in the calculations for each feature as opposed to a fishnet grid'}
...
```{r comparehexrectheat, message=FALSE,fig.cap='Comparing rectangular (A), hexagonal (B), and heat maps (C)'}

library(cowplot)

plot_grid(rectangular_bin_map, hexagonal_bin_map, heatmap, nrow =1, labels = "AUTO", scale = c(1, 1, .85))
...
```{r, fig.cap='EU referendum results across Local Authorit (no transformation)', fig.height=4}
ggplot() +
