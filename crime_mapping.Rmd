...
What more? You may also have seen the column "lsoa_name" seems to have some spatial component, Let's have a look at the first crime again. You see the value for LSOA name is `r print(crimes$lsoa_name[1])` - Bolton we know is a Borough of Greater Manchester, but what is the 001 for ?
...
To read  further in crime attractors vs crime generators turn to the recommended readings by @Brantingham_1995 and @Newton_2018. There have since been more developments, for example about crime radiators and absorbers as well (watch this risky places lecture from Kate @Bowers_2021 to learn more!)
...
manchester <- st_read("data/manchester")
...
leave_breaks <- quantile(eu_sf$Pct_Leave, 
                         probs = seq(0,1, by = 1/3), # vector of probabilities
                         na.rm=TRUE, # remove NAs before computing quantiles
                         names=TRUE, # result will have names attribute
                         include.lowest=TRUE) # include value equal to 
                                              # the lowest ‘breaks’ value 
```
...
```{r, dsplayallcolourbrewerpallettes, fig.height=6.5}
...
hu_dd_with_inset <- ggdraw() +  # set layer
  ...
  draw_plot(inset_map,   # draw inset map
            x = 0.67,    # specify location on x axis
