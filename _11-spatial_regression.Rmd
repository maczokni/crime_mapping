...
Given the "galaxy" of spatial models, newer approaches to select the one to run have been proposed. -@LeSage_2009 proposes a *top-down* approach that involved starting with the spatial Durbin model (SDM) and based on likelihood ratio tests to select the best model. -@Elhorst_2010, on the other hand, proposes to start with the OLS model, run the Lagrange multipliers, and then run a likelihood ratio test to see if the spatial Durbin model would be more appropriate.

If you paid attention the summary of the lag model we ran in the previous seciont included at the bottom a test for remaining residual spatial autocorrelation. This means our model is still not filtering all the spatial dependency that is present here. Perhaps here, we need to consider if the spatial Durbin model (SDM) would be more appropriate. We can run this model and then contrast it with the SAR model.
...
We can then compare the nested models with AIC and BIC. Using this information criteria, we choose the model with the lowest AIC and BIC.
...
Our results indicate that AIC favours the SDM model whereas the BIC favours the simpler SAR model. We can also run a likelihood ratio test to compare the two spatial nested models using `spatialreg::LR.Sarlm()` (that deprecates `spded::LR.sarlm()` you may still see mentioned in other textbooks):

```{r}
LR.Sarlm(fit4_70_sar, fit4_70_sdm)
...
Bla bla bla only superficial, havent covered details about estimation, bla bla bla... other approaches other packages...
