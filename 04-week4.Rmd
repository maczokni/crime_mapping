...
In this chapter we get your hands dirty with **spatial manipulation of your data**. Thus far, our data manipulation exercises were such that you might be familiar with, from your earlier exposures to data analysis. For example, linking datasets using a common column is a task which you can perform on spatial or non-spatial data. However today we will explore some exercises in data manipulation which are specific to *spatial* data analysis. 
...
Great OK so we have this list of licensed premises, and we have their address, which is clearly *some* sort of spatial information, but how would you put this on a map? Any ideas? 
...
### Accessing Open Street Map data with R (ref to the tutorial that inspired me for this)

Open Street Map (OSM) is to mapping what Wikipedia is to encyclopedias. It is as Wikipedia says "a collaborative project to create a free editable map of the world." OSM data is the premier source of freely available, community created geographic data worldwide. A number of packages, such as `osmdata` or the more recent and powerful `osmextract`, have been developed to make it easier to query and read data from OSM into R. Here we will first show you how to use `osmdata`, which is ideal for downloading small OSM datasets. This package uses an overpass API, which allows you to query the OSM database with your own criteria.
...
#### Build a query: where are police stations in BArcelona?

Before choosing what data to obtain from OSM you need to know what there is.
OSM data not only includes streets, roads and highways, it also includes information on other points of interest. The `available_features()` function from `osmdata` returns a list of available OSM features. You can find detail information on these different map features that you can obtain through OSM in their wiki pages (https://wiki.openstreetmap.org/wiki/Map_features).

```{r}
head(available_features())
```
Each of those features have associated tags as subcategories. So, for example, there are multiple types of what OSM calls amenities.

```{r}
head(available_tags("amenity"))
```

Police stations are tagged as amenities within OSM data. 

To build the data we will use the piping operator. In the first part of our query we need to indicate the area for which we want to obtain the information about the location of police stations. For this purpose we will use the `getbb( )` function, which creates a boundering box for a given place, looking for the name. The function `opq( )` builds the final query. The filter criteria for the query are included with the  function`add_osm_feature( )`. Since we are looking for police stations in Barcelona we specify "Barcelona" to bound the box and we use as key "amenity" and "police" as tag. 

```{r}
#building the query
query_1 <- getbb("Barcelona") %>%
      opq() %>%
       add_osm_feature(key = "amenity", 
                       value = "police")

str(query_1) #query structure
```

The resulting query can be obtained in various formats. We will ask for the query to result in a `sf` object by virtue of using the `osmdata_sf()` function.

```{r}
police <- osmdata_sf(query_1)
police
```

As we can see this produces various spatial objects. We are only interested in the points, which represent the location of these police stations. 

#### Let´s look at the data

We are going to use code you should be familiar from last chapter. With the `geom_sf( )` function introduced in the last chapter we will plot the locations of police stations in Madrid. It is important to indicate with the argument `inherit.aes = FALSE` that it has to use the aesthetic mappings of the spatial object osm_points. In addition, we change the color, fill, transparency (alpha), type and size of the circles.

```{r}

#police stations map
police_data <- police$osm_points
