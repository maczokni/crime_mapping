...
This package requires as well the centroids for the polygons we are using. To extract this we can use `sf::st_centroid()`. All the GEOS functions underlying `sf` need projected coordinates to work properly, so we need to reproject first to an adequate projection:
