...
This chapter aims to provide a basic introduction to RStudio, the interface we recommend to work with R, and to the basic data structures and grammar of the R programming language. By the end of the chapter you will:
...
+ know how to create an R project as the optimal way to organise your work in R,
+ learn how to install packages to expand the functionality of R,
+ learn how to use style guides to write better code,
...
You can think of R as a Lego monster. You can make the monster scarier and more powerful by adding new bits to it. Packages are those bits. They are modules that expand what R can do. There are thousands of them. That is down to the fact that researchers all over the world write packages that continuously expand the functionality of R. 
...
One of the key advantages of doing data analysis this way - with code versus with a point and click interface like Excel or ArcGIR is that you are producing a written record of every step you take in the analysis. First time around it will take you time to write these instructions, it may be slower than pointing and clicking. And unlike with pointing and clicking you need to know the "words" and "grammar" of this language. 
...
The R languages uses **functions** to tell the computer what to do. In the R *language* functions are the *verbs*. You can think of functions as predefined commands that somebody has already programmed into R *language* and tell the R *engine* what to do. Here you learnt your first R function: `print()`. All this function does is to ask R to print whatever it is you want in the main console (see the window in the bottom left corner). 
...
Technically R is an *Object Oriented language*. Object-oriented programming (OOP) is a programming language model organized around objects rather than "actions" and data rather than logic. As we have seen earlier, to create an object you have to give it a name, and then use the assignment operator (the `<-` symbol) to assign it some value. For example, if we want to create an object that we name "x", and we want it to represent the value of 5, we write:
...
A handy tip is to cut and paste the error message into Google and find a solution. If anybody had given us a penny for every time we had to do that ourselves, we would be as rich as Bill Gates by now. People make mistakes all the time. It's how we learn. Don't get frustrated, don't get stuck. Instead look for a solution. 
...
```{r}
the_beatles <- factor(c("John Lennon", "Paul McCartney", "Ringo Starr", "George Harrison")) #create a new factor
the_beatles #auto-print the factor
#Alternatively for similar result using the as.factor() function
the_beatles_bis <- c("John Lennon", "Paul McCartney", "Ringo Starr", "George Harrison") #create a character vector
the_beatles_f <- as.factor(the_beatles_bis) #create a factor using a character vector
the_beatles_f #auto-print factor
...
+ *You want to follow consistent naming conventions.* R users are terrible about this. But we could make it better if we all aim to follow similar conventions. In these handouts you will see I follow the `underscore_separated`.
...
Ok, so now that you understand some of the basic types of objects you can use in R, let's start taking about data frames. One of the most common objects you will work with in any project are **data frames**. Data frames can be created with the `data.frame()` function. 
...
mydata_1 <- data.frame(y = 1:4, x = c(T,T,F,F))
mydata_1
...
o_y <- 1:4
o_x <- c(T, T, F, F)
mydata_2 <- data.frame (y = o_y, x = o_x)
mydata_2
...
These results printed in the may console may not make too much sense to you at this point. We will return to this in later chapters.
