...
So let's read in our datasets, and join the attribute data to the geometry using `left_join()` (if you're unsure about any of the below code, revisit Chapter 1 of this book for a refresher!). 
...
```{r themmapratefill, fig.cap="Map with rate instead of count", fig.alt="The map of Hungary, shaded in blues similar to before. A legend to the right, labeled 'pos rate', matches lighter shades of blue to higher numbers. A vertical strip along the centre of the country is much lighter than the east or west."}
...
```{r, fig.height=6, fig.cap='Colour-blind friendly palettes in RColorBrewer package', fig.alt="Much like before, this is another list of names of colour palettes, which are still concatenated abbreviations of shortened colour names or a small descriptor. The names are yet again accompanied by a series of wide rectangles coloured in according to the palette."}
...
```{r newnewlabelsplot, warning=FALSE, message=FALSE, fig.cap='Re-write values on map legend', fig.alt="Further changes to the same map of Hungary, the ranges next to the colours now use the connecting word 'to', and 'less than' and 'greater than' signs where needed."}
...
```{r orientationarrowstyle, warning=FALSE, fig.cap='Change type of north arrow', fig.alt="The arrow above the letter 'N' is now longer, and bisects the letter."}
...
For general readings on cartography, the work of Kenneth Field provides thorough and engaging guidance (e.g., @Field_2012 or @Field_2018). For data visualisation, while @Tufte_1983 is a classic text, those looking for practical instruction may wish to turn to @Kirk_2016 or @Cairo_2016 for more guidance. 
