...
Data visualisation is a somewhat newer field, however, it seems to encompass the same guiding principles when considering what makes good design. According to @Kirk_2016 three principles offer a guide when deciding what makes a *good* data visualisation. It must be: 
...
We've covered a few approaches to mapping (using `ggplot2`, `tmap`, and `leaflet`) but here we will continue with `ggplot2` package to plot our thematic maps. We will map our sf objects using the `geom_sf()` function. To shade each polygon with the values of a specific variable, we use the `fill = ` argument within the `aes()` (aesthetics) function. Most simply: 

```{r themmap, fig.cap='Quick thematic map with ggplot2'}
...
```{r themmapthemevoid, fig.cap='Add theme_void() to ggplot map'}
...
```{r themmapwithoubord, fig.cap='Thematic map with border lines removed'}
...
```{r themmapwhitebord, fig.cap='Thematic map with white borders'}
...
```{r, fig.cap='Map of quantiles eith default colour scheme'}
...
```{r gradsymbmap, message=FALSE, warning=FALSE, fig.cap='Graduated symbol map'}
...
```{r gradsymbmapcol, message=FALSE, warning=FALSE, fig.cap='Graduated symbol map with adjusted colours'}
...
```{r gradsymbmapcoltriangle, message=FALSE, warning=FALSE, fig.cap='Graduated symbol map with triangle shape.'}
...
To use this function, we need our data to be in a dataframe which has separate columns for latitude and longitude (rather than a geometry object), where each observation has a unique identifier, and where we have a separate column for each proportion we wish to visualise. 
...
```{r, warning=FALSE, message=FALSE, fig.cap='Pie charts representing proportions on a map'}
...
Depending on the kind of varaible we want to visualise, we might want a Qualitative colour scheme (for categorical nominal variables), a Sequential colour scheme (for categorical ordinal, or for numeric variables) or a Diverging colour scheme (for categorical ordinal, or for numeric variables). 
...
```{r, dsplayallcolourbrewerpallettes, fig.height=7, fig.cap='Palettes in RColorBrewer package'}
...
```{r, fig.height=6, fig.cap='Colourblind friendly palettes in RColorBrewer package' }
...
```{r greyscaletestsmap, fig.cap="Map with greyscale palette"}
...
```{r, fig.cap="Map with monochrome palette"}
...
```{r legend, warning=FALSE, message=FALSE, fig.cap='Add title to map legend'}
...
```{r legendlabelsplot, warning=FALSE, message=FALSE, fig.cap='Edit labels on map legend'}
...
```{r newnewlabelsplot, warning=FALSE, message=FALSE, fig.cap='Rewrite values on map legend'}
...
```{r addcountynames, warning=FALSE, message=FALSE, fig.cap='Add labels to map'}
...
```{r, warning=FALSE, message=FALSE, fig.cap='Repel labels for legibility'}
...
```{r addcountynamestoponly, warning=FALSE, message=FALSE, fig.cap='Label only polygons in top quartile of value on variable of interest'}
...
```{r addcountynamesBPonly, warning=FALSE, message=FALSE, fig.cap='Label only one area of interest'}
...
```{r, warning=FALSE, fig.cap='Add arrow to clarify label off map'}
...
```{r, warning=FALSE, message=FALSE, fig.cap='Include production notes in map'}
...
```{r orientation, warning=FALSE, fig.cap='Add north arrow'}
...
```{r orientationarrowstyle, warning=FALSE, fig.cap='Change stype of north arrow'}
...
```{r scale, warning=FALSE, fig.cap='Include scale indicator on map'}
...
```{r basemap, warning=FALSE, message=FALSE, fig.cap='Add a basemap to our map'}
...
```{r insetmapfinal, fig.cap='Adding an inset map to our map'}
