...
In this chapter we will be working with some data published by Hungarian police available online [@Policehu_2021]. Specifically we will be looking at some statistics related to drink driving. Drunk driving is one of a number of problems police confront that relate to impaired and dangerous driving. Hungary has a strict drink driving policy, with the maximum drink diving limit being 0.0 BAC. Most European countries are at 0.5 BAC, while the UK is 0.8 (except 0.5 for Scotland). We have record for each county the number of brethalyser checks carried out, and the number of these which returned a positive result. 
...
So let's read in our data sets, and join the attribute data to the geometry using `left_join()`. 
...
hu_dd_with_inset <- ggdraw() +  # set layer
  ...
  draw_plot(inset_map,   # draw inset map
            x = 0.7,    # specify location on x axis
...
For general readings on cartography, the work of Kenneth Field privudes (eg: @Field_2012 or @Field_2018) thorough and engaging guidance. For data visualisation, while @Tufte_1983 is a classic text, those looking for practical instruction may wish to turn to @Kirk_2016 or @Cairo_2016 for more guidance. 
