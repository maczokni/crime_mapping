...
There are important pieces of information with every map which are represente by text. 
...
Or you want to specifically highlight a county of interest, let's say Budapest. In this case, we might actually want to keep our annotation off the map. We can do this byusing the `nudge_x` and `nudge_y` parameters of the `geom_sf_label()` function. 
...
ggplot(data = hu_dd) + 
  ...
  theme_minimal() + 
  ggtitle("Number of breathalyser tests per county", subtitle = "January 2020") + 
  labs(caption = "This map contains data from: Police Hungary available on: http://www.police.hu/hu/a-rendorsegrol/statisztikak/kozrendvedelem
       \n and OSM")
