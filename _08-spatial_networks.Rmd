...
- providing a quick overview of `sfnetwork`
...
```
So what we have is a `sf` object containing linestrings. But getting the data in this form is only the first step. For us to be able to do analysis on this network we need a clear topology. We need to move from linestrings to a network, with nodes and edges. This can be done by virtue of manual editing, which in large networks may be prohibitive, fully automatized, or automatized with manual edits just to correct errors. Explaining how to do this in detail with R would require space we do not have. Fortunately, there are some good tutorials out there that you can consult for this data pre-processing work.
...
- The vignettes for `sfnetwork` also cover how to turn a linestring `sf` object into a network. The function `as_sfnetwork()` provides the key functionality for this. It also includes several additional functions for cleaning the resulting network and there is a specific vignette documenting these.
- @Briz-Redón_2019a developed the `SpNetPrep` (Spatial Network Preprocessing). This package will open a Shiny application in RStudio that will allows for network creation and edition, network direction endowment and point pattern revision and modiﬁcation. The key driver of this package is to allow for manual edits of the constructed network to correct for errors.
...
bakerloo_line <- st_read("data/bakerloo_line.geojson", quiet=TRUE)
```
...
One way is to use the spatial information available with our data, and perform spatial operations such as building a buffer and subsetting only those points which fall within a buffer. See @Lovelace_2019 for more detail on the relevant functions and explanations for what is going on with each line.  
