...
For our demonstration, we will also need to select only those crimes which we can consider to have happened on (or near the Bakerloo Line). When carrying out your analysis you mayt have some flag or other variable in the data which indicates whether or not a crime occurred on or near a specific location. 

In the case of the police.uk data we have a column that we consider them to have happened on or near a specific location. This is the `location` column. If we have a glance you could see the sort of information it contains: 
...
One way is to use the spatial information available with our data, and perform spatial operations such as building a buffer and subsetting only those points which fall within a 200 meter buffer. See @Lovelace_2019 for more detail on the relevant functions and explanations for what is going on with each line.  
...
To do this, we can use the `st_nearest_feature()` function. This will return, for each point, the ID of the nearest segment. This function is also covered in greater detail in Chapter 2 of @Lovelace_2019 so we won't go into too much detail here. 
...
The corresponding Gini coefficient represents the ratio of the area between the line of perfect equality and the observed Lorenz curve to the area between the line of perfect equality and the line of perfect inequality (@gastwirth1972estimation). The closer the coefficient is to 1, the more unequal the distribution is (@zeileis2012packa9ge).
...
You can currently do this in R with at least two separate packages `spatstat` and the newer `spNetwork`. For details in the underpinning methodology for how each of these packages do this analysis see @Baddeley_2016 and the vignettes for `spNetwork` respectively. For reasons of space we will only cover `spatstat` here.
...
The `linnet` object will represent the network of a set of line segments, containing information about each of these segments (edges) and its vertices (nodes). You can generate a `linnet` object from a `sp::SpatialLines` object using functions from `maptool`, specifically `as.linnet.SpatialLines()`. If you need to process this network before is ready you can do so with some functions from `spatstat` or can use `sfnetworks` or `SpnetPrep` as preferable (and only then create your `linnet` object). 

@Briz-Redon_2019 offers an open access step by step illustration of how to transform a `SpatialLines` object into `linnet` and how, then, generate a `lpp` object. They do their demonstration using crime incidents extracted from the package `crimedata` (that we introduced in Chapter 7). We defer to this tutorial for details of how to do so. To save some time we will use here the `chicago` dataset that is distributed as part of the `spatstat` package. The `chicago` data is of class `lpp` and contains information on crime in an small area of Chicago. It is the street network we reproduced at the beginning of this chapter. You can plot it again if you don´t remember. For an analysis of this point pattern you can see @Ang_2012.
...
Environmental crimes could exist along waterways such as streams, canals, and rivers; and thefts of metal could occur along utility networks such as pipelines.

`stplanr`
`spnet`

The focus in this chapter has been to introduce the issue of crime events along a network. We have discussed some of the issues associated with the storage, handling, and visualisation of this kind of data. In the next two chapters we will return to the problem of assessing clustering and the detection of hot spots along street networks. 

Chapter 1 of -@Okabe_2012 provides a fairly accessible introduction to the relevance of point pattern data along a network, whereas Chapter 3 offers a slighty more technical discussion of the computational issues associated with the statistical analysis of points along a network. The first few sections of Chapter 17 of -@Baddeley_2016 also provides a bit background and provides very useful detail on the way that `spatstat` handles and stores point pattern data in a network. -@Baddeley_2021 offers an excellent overview of the challenges of analysing this kind of data and the available methods we have. This overview can also offers an excellent framework to understand the issues of clustering and detection of local clusters along networks to which we will return in the next two chapters.

for a book-length discussion of the issues associated with the statistical analysis of point pattern data along a network, as well as Chapter 
