...
```
So what we have is a `sf` object containing linestrings. But getting the data in this form is only the first step. For us to be able to do analysis on this network we need a clear topology, we often will need the data to be stored as a network graph. We need to move from linestrings to a graphical network representation, with nodes and edges. This can be done by virtue of manual editing, which in large networks may be prohibitive, fully automatized, or automatized with manual edits just to correct errors. Explaining how to do this in detail with R would require space we do not have. Luckily, there are some good tutorials out there that you can consult for this data pre-processing work.
...
We quickly below show the `as_sfnetwork()` function so that you get a sense for the look of a spatial network, but for further details on network construction and pre-processing it is important you see the documentation, as well as the other materials cited above.
...
You can currently do this in R with at least two separate packages `spatstat` and the newer `spNetwork`. For details in the underpinning methodology for how each these packages do this analysis see @Baddeley_2016 and the vignettes for `spNetwork` respectively. For reasons of space we will only cover `spatstat` here.
