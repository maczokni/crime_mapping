...
burg_moranmc_results <- moran.mc(cc_burglary_sp$burglary, 
                                 lw_queen, nsim=99)
...
We can use this test to check for our various specifications of the spatial weight matrix:
...
As we can see using the different specifications of neighbours produces results that are more suggestive of dependence.

We can also make a “Moran scatter plot” to visualize spatial autocorrelation. Note the row standardisation of the weights matrix.
...
```{r}
moran.plot(cc_burglary_sp$burglary, 
           wm_queen_rs)
```
