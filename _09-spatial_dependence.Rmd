...
In this chapter we are going to discuss ways in which you can quantify the answer to this question. We will start introducing methods to assess spatial clustering with point pattern data. Then, we will discuss measures of **global spatial autocorrelation** for lattice data, which essentially aim to answer the degree to which areas that are near each other tend to be more alike. We say global because we are interested in the degree of clustering not on the location of the clusters, this we will cover in the next chapter. 
...
manchester <- st_read(shp_name, quiet=TRUE)
...
burglary_sf <- st_as_sf(burglary, coords = c("long", "lat"), 
                 crs = 4326, agr = "constant")
burglary_sf <- st_transform(burglary_sf, 27700)
# Remove redundant non spatial burglary object
rm(burglary)

# Select burglaries that intersect with the Manchester city LSOA map.
bur_mc <- st_intersects(manchester, burglary_sf)
burglary <- burglary_sf[unlist(bur_mc),]
...
```{r, message=FALSE, warning=FALSE}
# Point in polygon spatial operation (this can take time)
burglary_lsoa <- burglary %>% 
  st_join(manchester, ., left = FALSE) %>% 
  count(code)
# Remove redundant objects
rm(burglary)
# Rename column with the count of burglaries (n)
burglary_lsoa <- rename(burglary_lsoa, burglary = n)
# Plot with tmap
tm_shape(burglary_lsoa) +
...
ward <- st_read("https://raw.githubusercontent.com/RUMgroup/Spatial-data-in-R/master/rumgroup/data/wards.geojson", quiet = TRUE) %>%
  filter(wd16nm == "City Centre")
# Intersect
cc_intersects <- st_intersects(ward, burglary_lsoa)
cc_burglary <- burglary_lsoa[unlist(cc_intersects),]
# Remove redundant objects
rm(ward)
rm(cc_intersects)
# Plot with tmap
...
```{r}
nb_queen <- poly2nb(bur_ccsp, row.names=bur_ccsp$lsoa_code)
class(nb_queen)
```
...
For more details we can look at the structure of "nb_bur".
...
```{r}
radius <- max(unlist(nbdists(nb_kn1, xy)))
radius
```
...
```{r}
nb_dist_1 <- dnearneigh(xy, 0, radius)
summary(nb_dist_1)
...
```{r}
distances <- nbdists(nb_dist_1, xy)
...
@Chi_2020 suggest that using the distance based spatial weight matrix tend to result in too few neighbours for rural areas and many neighbours for urban areas. In this case they point out that the k-nearest neighbourhood structure may be more appropriate.
...
[[[get the whole of manchester for this: it is below]]]
...
```{r}
geary.test(bur_ccsp$burglary, ww)
```
...
For a deeper understanding of the study of spatial dependence on point patterns we recommend @Baddeley_2016. @Haining_2020 and @Darmofal_2015 both contain useful chapters on the spatial weight matrix that provide a bit more detail than we do here, whilst maintaining an accessible level. We highly recommend too the series of videolectures provided by Luc Anselin in the YouTube channel of the Center for Spatial Data Science (University of Chicago). Prof. Anselin is behind the development of GeoDa, a point and click interface for spatial data analysis, and has been one of the key authors in the field of spatial econometrics during the last 25 years. His lectures are both high quality and accessible for someone starting in the field of spatial statistics. You will find lectures on the spatial weight matrix, global autocorrelation, and local indicators of spatial autocorrelation. The link for the relevant playlists is https://www.youtube.com/user/GeoDaCenter/playlists. In this Center website (spatial.uchicago.edu) you will also find R tutorials that provide instructions in how to specify the spatial weight matrix using other approaches we have not covered here (e.g., kernel weights, etc.).
