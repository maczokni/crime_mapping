...
For this exercise we travel to Buenos Aires, the capital and largest city in Argentina.  The city provides open data in geocoded crime from 2016 onwards. The data is freely available from the official source (http://mapa.seguridadciudad.gob.ar/). But we have added it already as supplementary material available from our GitHub repository. We will filter on "homicidios dolosos", intentional homicides and we are using all homicides from 2016. After reading the .csv file we turn it into a `sf` object and plot it. We are also reading a geojson file with the administrative boundary of Buenos Aires, that is already in EPGS:4326. The code below uses functions in ways we have demonstrated in earlier chapters.
...
buenos_aires <- st_read("data/buenos_aires.geojson")
...
We will be using the `spatstat` functionality, thus, need to turn this spatial point pattern data into a `ppp` object. Before doing so, we will transform the data into a projected coordinate system. We aill also generate a **convex hull** to be used as the`owin` object. In geography (and mathematics) the convex hull or envelop, "for a set of points X in a real vector space V is the minimal convex set containing X". For this we use `st_convex_hull` from `sf` in conjunction with `st_union` (to union the points as multipoints). Other than that we are using code already explained in Chapter 6. As we will see there are duplicate points. Since the available documentation does not mention any masking of the locations for privacy purposes, we cant assume that´s the reason for it. For simplicity purpose we will do as in Chapter 6 and introduce a small degree of jittering with a rather narrow radius.
...
For this we use the `spatstat::envelop` function. As arguments we use the data input, the function we are using (Kest), the number of simulations, and the parameters needed to obtain the global envelope.
...
We are going to take some of the data from past weeks. In getting the data ready you will have one more opportunity to practice how to read data into R but also how to perform some basic spatial checks, transformations and operations. First let's get the LSOA boundary data and change the coordinate system.
...
Let's add the burglary data from Greater Manchester. We have practiced this code in previous chapters so we won't go over it on detail again, but try to remember and understand what each line of code rather than blindly cut and paste. 
...
We now have the burglaries data, let's now count how many burglaries there are within each LSOA polygon. This is a point in polygon operation that we covered in chapter 2 when we counted the number of violent crimes in the buffers of the licenced premises for example. 
...
We will use familiar code to clip the spatial object with the counts of burglaries to only those that intersect with the City Centre ward. Again, we have covered this code elsewhere, so we won't explain here in detail. 
