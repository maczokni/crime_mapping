...
[[[Need to put in British grid]]]

We are going to take some of the data from past weeks. In getting the data ready you will have one more opportunity to practice how to read data into R but also how to perform some basic spatial checks, transformations and operations. First let's get the LSOA boundary data.
...
We now have the burglary data, let's now count how many burglaries there are within each LSOA polygon. This is a point in polygon operation that we covered in chapter 2 when we counted the number of violent crimes in the buffers of the licenced premises for example. 
...
Then we use the `spdep::knearneigh()` function to find the nearest neighbour to each centroid. Then we turn the created object into a `nb` object as defined earlier using `spdep::knn2nb`:
